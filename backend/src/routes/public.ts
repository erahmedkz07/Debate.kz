...
import { getStandings, getTournamentDetails, summaryInclude, toSummary } from '../services/tournaments.js'
...
publicRouter.get('/tournaments/:id/standings', async (req, res) => {
  const t = await prisma.tournament.findUnique({ where: { id: param(req, 'id') }, select: { visible: true } })
  if (!t?.visible) throw notFound('tournament_not_found')
  res.json(await getStandings(param(req, 'id')))
})

publicRouter.get('/cities', async (_req, res) => {
  const rows = await prisma.tournament.findMany({ where: { visible: true }, distinct: ['city'], select: { city: true }, orderBy: { city: 'asc' } })
...
publicRouter.get('/stats', async (_req, res) => {
  const [tournaments, teams, debaters, cities] = await Promise.all([
    ...
    prisma.tournament.findMany({ where: { visible: true }, distinct: ['city'], select: { city: true } }),
  ])
...
publicRouter.get('/rating', async (_req, res) => {
  const tournaments = await prisma.tournament.findMany({ where: { visible: true }, select: { id: true, level: true, city: true } })
